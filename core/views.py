...
class ChangePasswordView(FormView):
    ...
    def form_valid(self, form):
        ...
        self.request.user.isVerified = False
...
def get_priority_chart(user):
    chart_data = Bug.objects.filter(submitted_by=user).values("priority").annotate(count=Count("id"))
    labels = [str(data["priority"]) for data in chart_data]
